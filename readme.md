Claro! Aqui está o README com a formatação corrigida e melhor organizada:
...
1. **Leitura:** O Cardápio Digital lê o arquivo `js/dados.js` ao carregar a página para renderizar produtos e configurações.
2. **Escrita (Simulada):** O Painel de Gestão carrega o `dados.js`, permite edições na UI e gera um novo arquivo `dados.js` para download.
3. **Deploy:** O administrador substitui o arquivo antigo pelo novo no servidor para atualizar o cardápio.
...
- Ao encontrar o item, identifica a categoria pai (ex: "🥖 Pães Artesanais")
- Na renderização, agrupa automaticamente os itens sob seus respectivos títulos
- **Fallback:** Se o item não estiver na árvore, busca nas seções de produtos (permitindo vender produtos como adicionais, ex: "Alichella")
...
- **Modal Sticky Footer:** O `#container-subtotal-produto` utiliza `position: sticky` para garantir que o subtotal e botões de ação estejam sempre visíveis, independentemente do tamanho da lista de rolagem
...
2. Abra o `index.html` diretamente no navegador (ou use uma extensão como "Live Server" no VS Code para evitar bloqueios de CORS em alguns navegadores, embora o projeto seja desenhado para rodar via protocolo `file://` se necessário)

### Atualizando o Cardápio (Dia a Dia)

1. Abra `painelDeGestao-v3.2.html`
2. Carregue o arquivo `js/dados.js` atual
3. Faça as alterações (ex: mudar data da fornada, esgotar um produto)
4. Clique em "Salvar Alterações (Baixar JS)"
5. Substitua o arquivo `js/dados.js` antigo pelo novo arquivo baixado
6. Faça o commit/push para o repositório (se usar Git) ou upload para o servidor
